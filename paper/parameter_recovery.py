...
#-------------------------define functions-------------------------------------
def sample_param():
    log_sigma_rate = np.random.choice(np.linspace(-2., 2., 1000))
    log_a = np.random.choice(np.linspace(-2., 2., 1000))
    log_b = np.random.choice(np.linspace(-2., 2., 1000))
    log_beta = np.random.choice(np.linspace(-2., 2., 1000))

    params = [log_sigma_rate, log_a, log_b, log_beta]
...
def simulation(params):
    ...
    sigma_rate = np.exp(params[0])
    sigma_rate = np.repeat([sigma_rate], trial_num)

    exp_a = np.exp(params[1])
    exp_a = np.repeat([exp_a], trial_num)

    exp_b = np.exp(params[2])
    exp_b = np.repeat([exp_b], trial_num)

    beta = np.exp(params[3])
    beta = np.repeat([beta], trial_num)

    sigma_combine = sigma_rate/(1+exp_b*torch.exp(-exp_a*whole[:,0]))
    e_mean = (whole[:,2])/(sigma_combine + 1)
